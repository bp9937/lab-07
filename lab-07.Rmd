...
```{r scatterplot}
# add code here
evals %>% ggplot(mapping = aes(y = score)) + geom_boxplot()

evals %>% ggplot(mapping = aes(x = score, y = bty_avg)) + geom_point() + geom_jitter() + geom_smooth(method = "lm", se = FALSE, formula = y ~ x)
```

Jitter adds some randomness to your datasets, which is useful in handling overplotting, which may be cause by discreteness in smaller datasets.
...
evals %>% ggplot(mapping = aes(x = bty_avg, y = score)) + geom_jitter() + geom_smooth(method = "lm", se = FALSE, formula = y ~ x)
```
...
score_bty_aug %>%
ggplot(mapping = aes(x = .resid, y = .fitted)) + geom_jitter()
```
